...
from bert_loader import HTMLDataset 
from models import BERTClassifier 
from utils import save_plots, save_confusion_matrix 

MODEL_NAME = "bert-base-uncased" # The pre-trained BERT model to use
NUM_CLASSES = 4
MAX_LEN = 512          # Sequence length
BATCH_SIZE = 8
LEARNING_RATE = 2e-5   # Typically lower learning rates for fine-tuning pre-trained models
NUM_EPOCHS = 5         # Fewer epochs often sufficient for fine-tuning
WEIGHT_DECAY = 0.01 
GRADIENT_ACCUMULATION_STEPS = 4 
OUTPUT_DIR = "results_bert_classifier" # <--- New output directory name
...
def train_epoch(model, dataloader, optimizer, criterion, device, epoch):
    ...
    for batch_idx, (input_ids, attention_mask, labels) in enumerate(progress_bar): # Unpack attention_mask
...
def evaluate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        ...
        for input_ids, attention_mask, labels in progress_bar: # Unpack attention_mask
            input_ids, attention_mask, labels = input_ids.to(device), attention_mask.to(device), labels.to(device)
...
def main(csv_file):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "best_bert_classifier.pth"))
